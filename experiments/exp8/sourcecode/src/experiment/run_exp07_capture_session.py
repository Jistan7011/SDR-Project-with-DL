from __future__ import annotations

import argparse
import json
import random
from pathlib import Path

from src.common import load_config, write_json
from src.experiment.run_exp02_capture_session import run_session


def random_payload_pool(session_id: str, alphabet: str, count: int = 10, min_bytes: int = 1, max_bytes: int = 2) -> list[str]:
    seed = sum(ord(ch) for ch in session_id)
    rng = random.Random(seed)
    payloads: list[str] = []
    while len(payloads) < count:
        width = rng.randint(min_bytes, max_bytes)
        payload = "".join(rng.choice(alphabet) for _ in range(width))
        if payload not in payloads:
            payloads.append(payload)
    return payloads


def run_exp07_capture_session(
    session_id: str,
    output_root: str = "../data/raw_iq",
    config_path: str = "../config/config.exp07.yaml",
    radioconda_root: str = r"C:\Users\qus70\radioconda",
    distance_m: float = 1.0,
    antenna_layout: str = "face_to_face",
    antenna_note: str = "",
    tx_vga_gain: float = 30.0,
    rx_gain: float = 30.0,
    baseband_offset_hz: float = 500_000.0,
    tx_seconds: float = 5.0,
    capture_seconds: float = 10.0,
    noise_seconds: float = 3.0,
    dry_run: bool = False,
) -> Path:
    cfg = load_config(config_path)
    exp_cfg = cfg.get("experiment7", {})
    payloads = random_payload_pool(
        session_id,
        str(exp_cfg.get("payload_alphabet", "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")),
        count=10,
        min_bytes=min(int(x) for x in exp_cfg.get("payload_bytes_candidates", [1, 2])),
        max_bytes=max(int(x) for x in exp_cfg.get("payload_bytes_candidates", [1, 2])),
    )
    session_dir = run_session(
        session_id=session_id,
        output_root=output_root,
        config_path=config_path,
        radioconda_root=radioconda_root,
        payloads=payloads,
        baseband_offset_hz=baseband_offset_hz,
        tx_vga_gain=tx_vga_gain,
        rx_gain=rx_gain,
        tx_seconds=tx_seconds,
        capture_seconds=capture_seconds,
        noise_seconds=noise_seconds,
        dry_run=dry_run,
    )
    if dry_run:
        return session_dir
    metadata_path = session_dir / "metadata.json"
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    metadata.update(
        {
            "experiment": "exp7_unknown_protocol_absorption_guard",
            "purpose": "Unknown-protocol signal analysis and BASK absorption mitigation",
            "rf_path": "ota_antenna",
            "rf_cable_between_sdr": False,
            "protocol_known_to_analyzer": False,
            "payloads_randomized": True,
            "tx_rx_distance_m": float(distance_m),
            "antenna_layout": antenna_layout,
            "antenna_note": antenna_note,
            "line_of_sight": True,
        }
    )
    write_json(metadata_path, metadata)
    print(f"Experiment 7 metadata updated: {metadata_path}")
    return session_dir


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--session-id", required=True)
    parser.add_argument("--output-root", default="../data/raw_iq")
    parser.add_argument("--config", default="../config/config.exp07.yaml")
    parser.add_argument("--radioconda-root", default=r"C:\Users\qus70\radioconda")
    parser.add_argument("--distance-m", type=float, default=1.0)
    parser.add_argument("--antenna-layout", default="face_to_face")
    parser.add_argument("--antenna-note", default="")
    parser.add_argument("--tx-vga-gain", type=float, default=30.0)
    parser.add_argument("--rx-gain", type=float, default=30.0)
    parser.add_argument("--baseband-offset-hz", type=float, default=500_000.0)
    parser.add_argument("--tx-seconds", type=float, default=5.0)
    parser.add_argument("--capture-seconds", type=float, default=10.0)
    parser.add_argument("--noise-seconds", type=float, default=3.0)
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    run_exp07_capture_session(
        session_id=args.session_id,
        output_root=args.output_root,
        config_path=args.config,
        radioconda_root=args.radioconda_root,
        distance_m=args.distance_m,
        antenna_layout=args.antenna_layout,
        antenna_note=args.antenna_note,
        tx_vga_gain=args.tx_vga_gain,
        rx_gain=args.rx_gain,
        baseband_offset_hz=args.baseband_offset_hz,
        tx_seconds=args.tx_seconds,
        capture_seconds=args.capture_seconds,
        noise_seconds=args.noise_seconds,
        dry_run=args.dry_run,
    )


if __name__ == "__main__":
    main()
