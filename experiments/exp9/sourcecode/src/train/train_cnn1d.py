...
def train(
    ...
    if init_checkpoint:
        ...
        partial = bool(finetune_cfg.get("partial_load", False))
        load_initial_checkpoint(model, init_path, partial=partial)
...
def load_initial_checkpoint(model: nn.Module, checkpoint_path: Path, partial: bool = False) -> None:
...
def adapt_state_dict_for_model(target: dict[str, torch.Tensor], source: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    for key, target_tensor in target.items():
        ...
        if tuple(source_tensor.shape) == tuple(target_tensor.shape):
            ...
            continue
        if source_tensor.ndim == 3 and target_tensor.ndim == 3 and source_tensor.shape[0] == target_tensor.shape[0] and source_tensor.shape[2] == target_tensor.shape[2]:
            merged = target_tensor.clone()
            common = min(source_tensor.shape[1], target_tensor.shape[1])
            merged[:, :common, :] = source_tensor[:, :common, :]
            if target_tensor.shape[1] > source_tensor.shape[1]:
                filler = source_tensor.mean(dim=1, keepdim=True)
                merged[:, common:, :] = filler.expand(-1, target_tensor.shape[1] - common, -1)
            adapted[key] = merged
    return adapted
