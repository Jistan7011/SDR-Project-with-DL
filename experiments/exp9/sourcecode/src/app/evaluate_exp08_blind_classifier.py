...
def evaluate_exp08_blind_classifier(
    checkpoint: str,
    config_path: str,
    data_root: str,
    output_dir: str,
    ...
    model = build_model(
        str(ckpt.get("model_type", model_cfg.get("type", "multitask_resnet1d"))),
...
def write_prediction_csv(path: Path, files: list[Path], y_true: list[int], y_pred: list[int], confidences: list[float], margins: list[float]) -> None:
    ...
    lines = ["file,expected,predicted,confidence,margin,session_id,payload,snr_db"]
...
def main() -> None:
    ...
    evaluate_exp08_blind_classifier(args.checkpoint, args.config, args.data_root, args.output_dir, args.split)
