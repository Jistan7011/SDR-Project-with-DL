...
from src.signal.evidence import compute_signal_evidence, evidence_vector
...
def canonicalize_iq(
    ...
    evidence = compute_signal_evidence(time_features[:2])
    metadata = {
    ...
    return CanonicalizationResult(
        ...
        evidence_feature=np.nan_to_num(evidence_vector(evidence), nan=0.0, posinf=0.0, neginf=0.0),
        metadata=metadata,
    )
...
def estimate_phase_slope(iq: np.ndarray) -> float:
    ...
    if not np.any(np.isfinite(step)):
        return 0.0
    return float(np.median(step[np.isfinite(step)]))
