...
def calibrate(
    ...
    write_json(result_root / "exp05_class_bias_calibration.json", results)
    write_markdown(result_root / "exp05_class_bias_calibration.md", results)
    return results


def write_markdown(path: Path, results: dict[str, Any]) -> None:
    lines = [
        "# Experiment 5 Class Bias Calibration",
        "",
