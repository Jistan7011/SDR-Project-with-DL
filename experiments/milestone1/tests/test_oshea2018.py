...
from src.analysis.summarize_oshea2018_stage1 import bpsk_all_to_bask, summarize, worst_recall
...
def test_oshea2018_extension_models_forward() -> None:
    x = torch.randn(4, 5, 1024)
    resnet = build_model("resnet1d_5ch", input_channels=5, num_classes=3, dropout=0.1)
    assert resnet(x).shape == (4, 3)
    multitask = build_model("multitask_resnet1d_5ch", input_channels=5, num_classes=3, dropout=0.1)
    outputs = multitask(x)
    assert outputs["logits"].shape == (4, 3)
    assert outputs["bask_binary_logits"].shape == (4, 2)
    assert outputs["bfsk_bpsk_logits"].shape == (4, 2)
    assert outputs["embedding"].shape[0] == 4


def test_oshea2018_five_channel_feature_mode(tmp_path: Path) -> None:
    ...
    ds = IQDataset(tmp_path, "train", feature_mode="iq_mag_ifreq_dphase")
    x, y = ds[0]
...
def test_oshea2018_rf_canonicalization_shape() -> None:
    rng = np.random.default_rng(17)
    channels = rng.normal(size=(2, 1024)).astype(np.float32)
    result = canonicalize_iq(channels, sample_rate=2.4e6)
    assert result.time_features.shape == (5, 1024)
    assert result.spectral_feature.shape == (128,)
    assert np.isfinite(result.time_features).all()
...
def test_stage1_summary_derives_worst_recall_and_blocks_bpsk_collapse(tmp_path: Path) -> None:
    data = {
        "accuracy": 2 / 3,
        "macro_f1": 5 / 9,
        ...
        "confusion_matrix": [[5, 0, 0], [0, 5, 0], [5, 0, 0]],
        "bfsk_bpsk_to_bask_rate": 0.5,
    }
    assert worst_recall(data) == 0.0
    assert bpsk_all_to_bask(data) is True

    eval_dir = tmp_path / "results" / "fixed_quick_ota_resnet" / "logs"
    eval_dir.mkdir(parents=True)
    (eval_dir / "eval_test.json").write_text(
        json.dumps(data),
        encoding="utf-8",
    )
    summary = summarize(str(tmp_path / "results"), "fixed_quick_", str(tmp_path / "summary.json"), min_bpsk_recall=0.30)
    assert summary["stage2_allowed"] is False
    assert summary["rows"][0]["stage2_pass"] is False
    assert summary["rows"][0]["bpsk_all_to_bask"] is True
