...
class MultiTaskResNet1DClassifier(nn.Module):
    def __init__(self, input_channels: int = 5, num_classes: int = 3, dropout: float = 0.35, embedding_dim: int = 128):
        super().__init__()
        self.stem = conv_block(input_channels, 32, kernel_size=7)
        self.features = nn.Sequential(
            ResidualBlock1D(32, 32),
        ...
        self.bask_binary_head = nn.Linear(embedding_dim, 2)
        self.bfsk_bpsk_head = nn.Linear(embedding_dim, 2)

    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        z = self.embedding(self.pool(self.features(self.stem(x))))
        return {
...
def build_model(model_type: str, input_channels: int = 2, num_classes: int = 3, dropout: float = 0.3) -> nn.Module:
    ...
    if key in {"resnet1d_5ch", "rf_preprocessed_resnet1d"}:
        return ResNet1DClassifier(input_channels=input_channels, num_classes=num_classes, dropout=dropout)
    if key in {"multitask_resnet1d", "multitask_resnet1d_5ch", "multitask_resnet1d_margin", "multitask_resnet1d_margin_5ch"}:
