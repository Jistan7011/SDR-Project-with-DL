...
- checkpoint: `experiments/exp1/results/checkpoints/best.pt`
...
- HackRF One TX와 RTL-SDR RX는 유선 RF 케이블로 연결하지 않았다.
- 두 장비 모두 안테나를 사용한 OTA 송수신으로 실험했다.
- 두 안테나는 약 `10 cm` 거리에서 옆에 나란히 세웠다.
- 따라서 실험 1 결과는 유선 직결 baseline이 아니라 `10 cm 근거리 OTA baseline`이다.
...
`sourcecode/`에는 실험 1을 고정한 시점의 `src/` 코드가 들어 있고, `tests/`에는 당시 테스트 snapshot이 들어 있다. 이후 코드 변경으로 실험 2가 깨질 경우 참고용으로 사용한다.

`manifest.json`에는 보존 파일 목록, 크기, SHA256 hash, 기준 metric, 재현 명령이
들어 있다.

구조 변경 후 실제 HackRF TX + RTL-SDR RX 송수신까지 포함한 동작 확인 결과는 `TEST_REPORT.md`에 기록했다.
...
```powershell
.\.venv\Scripts\python.exe -m src.train.evaluate --checkpoint experiments/exp1/results/checkpoints/best.pt --data-root experiments/exp1/data/processed
```
...
```powershell
.\.venv\Scripts\python.exe -m src.train.train_cnn1d --config experiments/exp1/config/config.exp01.yaml --preset train
```
...
Copy-Item -Recurse -Force experiments\exp1\data\processed\* data\real\processed\

Copy-Item -Force experiments\exp1\data\raw_iq\bask_a.bin data\real\raw_iq\bask_a.bin
Copy-Item -Force experiments\exp1\data\raw_iq\bfsk_f.bin data\real\raw_iq\bfsk_f.bin
Copy-Item -Force experiments\exp1\data\raw_iq\bpsk_p.bin data\real\raw_iq\bpsk_p.bin

Copy-Item -Force experiments\exp1\results\checkpoints\best.pt results\checkpoints\best.pt
```
...
- 모든 실제 SDR capture는 안테나 OTA 방식으로 진행되었고, 두 안테나는 약 `10 cm`
  근거리에서 나란히 배치되었다. 따라서 실험 1의 성능은 `10 cm OTA 조건`에 대한
  baseline으로 해석해야 한다.
