...
def run_one(
    ...
    capture_cmd = radioconda_cmd(
        radioconda_root,
        "src.sdr.capture_iq",
        ["--output", str(output), "--seconds", str(capture_seconds)],
    )
    tx_cmd = radioconda_cmd(
        radioconda_root,
        "src.sdr.hackrf_tx",
        ["--modulation", modulation, "--payload", payload, "--seconds", str(tx_seconds)],
    )
...
def import_capture(input_path: Path, modulation: str, payload: str, import_seconds: float, import_skip: float, max_windows: int) -> None:
    cmd = [
        sys.executable,
        "-m",
...
def main() -> None:
    ...
    for modulation, payload, filename in DEFAULT_SEQUENCE:
        ...
        if args.import_after and not args.dry_run:
            import_capture(output, modulation, payload, args.import_duration_seconds, args.import_skip_seconds, args.max_windows)
